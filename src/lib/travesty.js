// originally ganked from https://raw.githubusercontent.com/javazen/travesty/master/src/travesty.js
const DEBUG = false
const CHARS = "abcdefghijklmnopqrstuvwxyz 'ABCDEFGHIJKLMNOPQRSTUVWXYZ,./<>!~!@#$%^&*();':`|[]{}1234567890-+=\""

export default function transform (str, order) {
...
function level2 (charsArr, str) {
  ...
  let n; let pos; let ch; let newstr = 'L2:  '
  ...
  for (let i = 1; i < len; i++) {
    const prevpos = pos
    const rowtot = rowTotals[prevpos]
...
function level3 (charsArr, str) {
  ...
  const fn = element => element >= n
  let n; let ch; let prevch; let prevprevch; let pos; let prevpos; let prevprevpos; let newstr = 'L3:  '
  ...
  const rowtot = rowTotals2[prevprevpos]
  n = Math.ceil((rowtot) * Math.random())
  ...
  for (let i = 2; i < len; i++) {
    const rowtot = rowTotals3[prevprevpos][prevpos]
    n = Math.ceil((rowtot) * Math.random())
